...
import { AlertTriangle, LayoutDashboard, Package, RefreshCw, ShoppingCart, Warehouse } from "lucide-react";
...
export function StoreSwitcher() {
  ...
  if (loading) return <span className="text-caption text-muted">加载店铺...</span>;
  if (stores.length === 0) {
    return (
      <Link href="/settings/stores" className="text-caption text-accent-violet-mid hover:underline">
        绑定店铺
      </Link>
    );
  }
...
export function TrackingShell({ children }: { children: React.ReactNode }) {
  ...
  const { stores, activeStoreId, refreshStores, notifyDataRefresh } = useStoreContext();
    ...
    <div className="max-w-7xl mx-auto px-xxl py-xxl">
      <header className="mb-xl flex flex-wrap items-start justify-between gap-md">
        <div>
          ...
          <h1 className="font-display font-bold text-heading-md text-ink mb-xs">店铺工作台</h1>
        ...
        {stores.length > 0 && (
          <Button variant="ghost" size="sm" onClick={handleSync} disabled={syncing}>
            <RefreshCw className={`h-4 w-4 mr-xs ${syncing ? "animate-spin" : ""}`} />
            {syncing ? "同步中..." : "立即同步"}
          </Button>
        )}
      </header>
      {syncError && (
        <p className="text-caption text-accent-pink mb-md -mt-lg">{syncError}</p>
      )}
