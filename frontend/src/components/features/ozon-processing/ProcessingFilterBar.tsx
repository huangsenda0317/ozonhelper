  ...
  const fields = (
    <>
      <div className="flex flex-col gap-xs flex-1 min-w-[10rem]">
        <label htmlFor="proc-keyword" className="text-caption text-muted">
          关键词
        </label>
        <input
          id="proc-keyword"
      ...
      <div className="flex flex-col gap-xs min-w-[7rem]">
        ...
        <Select
          ...
          onChange={(v) =>
            onChange({
              ...filters,
              source_platform: (v != null ? String(v) : "all") as ProcessingFilters["source_platform"],
            })
          }
          options={PROCESSING_SOURCE_OPTIONS}
          className={FILTER_FIELD_CLASS}
        />
      </div>
      <div className="flex flex-col gap-xs min-w-[8rem]">
        <label className="text-caption text-muted">创建时间</label>
        <Select
          value={filters.created_time === "all" ? undefined : filters.created_time}
          onChange={(v) =>
            onChange({
              ...filters,
              created_time: (v != null ? String(v) : "all") as ProcessingFilters["created_time"],
            })
          }
          options={PROCESSING_TIME_OPTIONS.filter((o) => o.value !== "all")}
          placeholder="全部"
          allowClear
          className={FILTER_FIELD_CLASS}
        />
