...
后端 SHALL 提供 `GET /api/v1/tracking/products/{product_id}`，需 JWT 认证。`product_id` 为 Ozon 数字 product_id。
...
- **WHEN** 请求的 `product_id` 在 Ozon 店铺中不存在
...
前端 SHALL 在 `/tracking/[productId]` 提供商品详情页，包含：
- 返回列表链接
- 主图与图片画廊（若有多个）
...
- **WHEN** 用户在列表页点击某商品
- **THEN** 导航至 `/tracking/{product_id}` 并展示该商品详情
