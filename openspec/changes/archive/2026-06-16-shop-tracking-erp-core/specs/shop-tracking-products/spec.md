...
- `is_exception`（可选，boolean）：筛选异常商品
...
- `realtime`（可选，boolean）：true 时绕过本地索引，实时调用 Ozon API（兼容 MVP 行为）

响应 MUST 使用 `ApiResponse[list[TrackingProductSummary]]` 并含 `PaginationMeta`（total、page、limit）。

每条摘要至少包含：`product_id`、`offer_id`、`sku`、`name`、`price`、`currency`、`stock_present`、`status_name`、`primary_image_url`、`updated_at`、`ordered_units`、`hits_view`、`conversion_rate`、`synced_at`、`is_exception`。
