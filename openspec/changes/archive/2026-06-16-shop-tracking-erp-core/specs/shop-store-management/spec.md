...
后端 SHALL 提供 `DELETE /api/v1/stores/{store_id}`，软删除或硬删除店铺及关联同步数据（按用户确认）。
...
所有 tracking 域 API SHALL 接受 `store_id` Query 参数；未传时使用用户最近活跃店铺。

用户 MUST 只能访问自己 `user_id` 下的店铺数据（FR-039）。
...
- **WHEN** 用户填写有效凭证并提交
- **THEN** 列表刷新，显示新店铺，并自动设为当前活跃店铺

### Requirement: env 凭证自动迁移

系统 SHALL 在 bootstrap 时将 `backend/.env` 中已有的 `OZON_CLIENT_ID`/`OZON_API_KEY` 为 admin 用户创建默认 Store（若该用户尚无店铺）。

#### Scenario: 首次启动迁移

- **WHEN** 服务启动且 env 凭证已配置、用户无店铺
- **THEN** 自动创建名为「默认店铺」的 Store 记录
