...
本地开发可通过 `SYNC_INLINE=true` 在 API 进程内后台执行同步。
...
- **WHEN** 用户点击「立即同步」
- **THEN** 创建 SyncJob 记录，入队任务，返回 job id

### Requirement: 同步任务状态 API

后端 SHALL 提供 `GET /api/v1/tracking/sync-jobs/{job_id}`，返回：
`status`（pending/running/succeeded/failed）、`records_processed`、`error_message`。
...
`sync_jobs` 表 SHALL 记录每次同步的执行元数据。
...
同步任务 MUST 遵守 Ozon per-Client-Id 请求频率控制；遇 429 MUST 指数退避重试。

#### Scenario: 429 重试

- **WHEN** 单次请求返回 429
- **THEN** 等待 backoff 后重试，失败后标记 job failed
