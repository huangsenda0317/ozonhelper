...
系统 SHALL 通过同步任务调用 `POST /v4/product/info/stocks` 同步库存至 `inventory_snapshots` 表，字段含：
...
单次请求 MUST 不超过 Ozon 官方上限，超出时后端分批执行。
...
系统 MUST 在同步后检测 `present < threshold` 并写入 `alerts` 表。
